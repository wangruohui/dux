# 当前任务：S3 Viewer 元数据与缓存聚合

当前目标：S3 Viewer 增加日期，并在浏览子 prefix 后把缓存中的递归已知大小展示到父级。

交付物：对象 `LastModified`、精确/部分目录大小、分页验证、删除后的缓存一致性测试与 README。

锁定对象：`/mnt/afs/wangruohui/tools/dux`；不读取或修改凭据内容，不改变本地文件系统 UI 行为。

阶段：DONE。

下一步：完成单元与 headless TUI 测试，提交并推送。
